...
/// <summary>WO-48: stub adapter for servicenow (MVP — returns empty results).</summary>
public sealed class ServiceNowAdapter : IConnectorAdapter
{
    public string ConnectorTypeId => "servicenow";
    ...
    public NormalizedEvent NormalizeEvent(string rawPayload, string connectorId, string tenantId)
        => new() { EventType = "servicenow", ConnectorId = connectorId, TenantId = tenantId, Payload = rawPayload };
}
