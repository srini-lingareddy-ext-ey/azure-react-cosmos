...
/// <summary>WO-48: stub adapter for oracle (MVP — returns empty results).</summary>
public sealed class OracleAdapter : IConnectorAdapter
{
    public string ConnectorTypeId => "oracle";
    ...
    public NormalizedEvent NormalizeEvent(string rawPayload, string connectorId, string tenantId)
        => new() { EventType = "oracle", ConnectorId = connectorId, TenantId = tenantId, Payload = rawPayload };
}
