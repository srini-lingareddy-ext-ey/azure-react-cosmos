...
/// <summary>WO-48: stub adapter for talend (MVP — returns empty results).</summary>
public sealed class TalendAdapter : IConnectorAdapter
{
    public string ConnectorTypeId => "talend";
    ...
    public NormalizedEvent NormalizeEvent(string rawPayload, string connectorId, string tenantId)
        => new() { EventType = "talend", ConnectorId = connectorId, TenantId = tenantId, Payload = rawPayload };
}
