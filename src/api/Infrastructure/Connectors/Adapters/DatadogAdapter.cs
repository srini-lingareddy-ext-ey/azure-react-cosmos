...
/// <summary>WO-48: stub adapter for datadog (MVP — returns empty results).</summary>
public sealed class DatadogAdapter : IConnectorAdapter
{
    public string ConnectorTypeId => "datadog";
    ...
    public NormalizedEvent NormalizeEvent(string rawPayload, string connectorId, string tenantId)
        => new() { EventType = "datadog", ConnectorId = connectorId, TenantId = tenantId, Payload = rawPayload };
}
