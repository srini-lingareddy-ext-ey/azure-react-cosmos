...
public sealed class NotificationsController : ControllerBase
{
    ...
    [HttpGet("delivery-logs")] public async Task<ActionResult<List<NotificationDeliveryLogDto>>> GetDeliveryLogsAsync([FromQuery] string? eventId, [FromQuery] int limit = 50, CancellationToken ct = default) => Ok(await _service.GetDeliveryLogsAsync(_tenantContext.TenantId, eventId, limit, ct).ConfigureAwait(false));
